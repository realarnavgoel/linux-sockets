...
//	do{
		...
		msg = buffer;
		if((nbytes = sendto(sock_fd,msg,strlen(msg),0,(struct sockaddr *)&serveraddr,serveraddrlen)) <= 0)
		{	
		...
		//Receive a message
		if((nbytes = recvfrom(sock_fd,buffer,255,0,(struct sockaddr *)&(serveraddr),&serveraddrlen)) > 0)
		{
